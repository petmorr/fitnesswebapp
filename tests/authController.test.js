...
describe('AuthController', () => {
  ...
  describe('Login and Session Management', () => {
    it('should handle successful login and fetch current user data', async () => {
        const loginResponse = await agent.post('/api/login').send({ email: 'user@example.com', password: 'password' });
        expect(loginResponse.status).toBe(200); // Verify if login was successful
        if (loginResponse.headers['set-cookie']) {
            const cookie = loginResponse.headers['set-cookie'][0];
            const userResponse = await agent.get('/api/current-user').set('Cookie', cookie);
            expect(userResponse.status).toBe(200); // Verify if user data can be fetched post-login
        } else {
    ...
    it('should reject login attempt with nonexistent user', async () => {
      const response = await agent.post('/api/login').send({ email: 'unknown@example.com', password: 'password' });
      expect(response.status).toBe(400);
    ...
    it('should reject login attempt with incorrect password', async () => {
      ...
      const response = await agent.post('/api/login').send({ email: 'user@example.com', password: 'wrongpassword' });
      expect(response.status).toBe(400);
      expect(response.body.errorMessage).toBe('Invalid password.');
    });

    it('should deny access when fetching user data without session', async () => {
        // Use a new agent without the cookie/session
        const newAgent = request.agent(app);
        const response = await newAgent.get('/api/current-user');
        expect(response.status).toBe(401);
        expect(response.body.errorMessage).toBe('User not authenticated');
    });
    
    it('should ensure session persistence after logout', async () => {
        const loginResponse = await agent.post('/api/login').send({ email: 'user@example.com', password: 'password' });
        expect(loginResponse.status).toBe(200);
        if (loginResponse.headers['set-cookie']) {
            const cookie = loginResponse.headers['set-cookie'][0];
            await agent.get('/logout').set('Cookie', cookie);
            const response = await agent.get('/api/current-user').set('Cookie', cookie);
            expect(response.status).toBe(401); // Expect unauthorized after logout
        } else {
            throw new Error('No cookie set after login');
        }
    });
